...
inline PadExecuteClient::PadExecuteClient(
    const std::string & name,
    rclcpp::node_interfaces::NodeBaseInterface::SharedPtr node_base_interface,
    rclcpp::node_interfaces::NodeGraphInterface::SharedPtr node_graph_interface,
    rclcpp::node_interfaces::NodeLoggingInterface::SharedPtr node_logging_interface,
...
inline bool PadExecuteClient::wait_for_action_server_available()
{
...
inline bool PadExecuteClient::send_goal(
    const std::string & pad_name,
    uint8_t action)
{
...
inline void PadExecuteClient::goal_response_callback(const GoalHandlePtr & goal_handle)
{
    if (!goal_handle) {
...
inline void PadExecuteClient::feedback_callback(
    GoalHandlePtr goal_handle,
    const std::shared_ptr<const ActionT::Feedback> feedback)
{
    (void)goal_handle;
...
inline void PadExecuteClient::result_callback(const GoalHandleT::WrappedResult & result)
{
