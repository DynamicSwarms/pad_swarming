...
class ChargeController:
    ...
    def __init__(
        ...
        cf_prefix: str,
        on_charged_callback: Callable[[], None],
    ):
        self._node = node
        ...
        if True:  # cf_type == CrazyflieType.HARDWARE:
            self._battery_subscription = self._node.create_subscription(
                msg_type=GenericLogData,
                topic=cf_prefix + "/state",
            ...
            self._fully_charged_timer = self._node.create_timer(
                1.0, self._webots_pub_charged_callback
            )
            self.voltage = 4.2  # A hardware crazyflie is always charged.

    def destroy_subscriptions(self):
        if True:  # self._cf_type == CrazyflieType.HARDWARE:
    ...
    def battery_voltage_empty(self):
        return (
            self._node.get_parameter("battery_voltage_empty")
            .get_parameter_value()
            .double_value
        )
    ...
    def battery_voltage_charged(self):
        return (
            self._node.get_parameter("battery_voltage_charged")
            .get_parameter_value()
            .double_value
        )
