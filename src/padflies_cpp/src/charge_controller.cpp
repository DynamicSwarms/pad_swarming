...
void 
ChargeController::set_on_charged_callback(std::function<void()> callback)
{
...
bool 
ChargeController::is_charged() const
{
    return m_battery_voltage >= get_battery_voltage_charged();
}

bool 
ChargeController::is_empty() const
{
    return m_battery_voltage <= get_battery_voltage_empty();
}

bool 
ChargeController::is_critical() const
{
    return m_battery_voltage <= get_battery_voltage_critical();
}

double 
ChargeController::get_battery_voltage() const
{
    return m_battery_voltage;
}

void 
ChargeController::on_battery_data(
...
double 
ChargeController::get_battery_voltage_charged() const
{
    return m_param_iface->get_parameter("battery_voltage_charged").as_double();
}

double 
ChargeController::get_battery_voltage_empty() const
{
    return m_param_iface->get_parameter("battery_voltage_empty").as_double();
}

double 
ChargeController::get_battery_voltage_critical() const
{
