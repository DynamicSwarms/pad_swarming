...
class AgentConnector:
    def __init__(
        ...
        self._node.declare_parameter(name="hardware_only", value=False)
        self._node.declare_parameter(name="webots_only", value=False)
    ...
    def _transition_event_callback(self, event: TransitionEvent):
        if event.goal_state.id == LifecycleState.TRANSITION_STATE_DEACTIVATING:
            self._node.get_logger().info("Caught transition event deactivating -> Disconnected.")
    ...
    def _disconnect(self):
        ...
        if self.is_connected():
            self.__transition_padflie_async(prefix=self.__padflie_prefix, state=LifecycleState.TRANSITION_STATE_DEACTIVATING, label="deactivate")
    ...
    def __transition_padflie(self, prefix: str, state: LifecycleState, label: str) -> bool:
    ...
    def __transition_padflie_async(self, prefix: str, state: LifecycleState, label: str) -> Future: 
