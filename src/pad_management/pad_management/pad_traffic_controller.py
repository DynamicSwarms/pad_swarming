...
class TrafficController(Node):
    def __init__(self):
        ...
        self.cancel_request: "dict[str, Event]" = {}
    ...
    def acquire_callback(
        ...
        self.cancel_request[name] = Event()

        while not self.cancel_request[name].is_set() and timeout:
            timeout -= 0.1
            if self.pad_lock.acquire(timeout=0.1):
