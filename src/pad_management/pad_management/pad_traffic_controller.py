...
class TrafficController(Node):
    ...
    def acquire_callback(self, request: PadRightAcquire.Request, response: PadRightAcquire.Response):
    ...
    def release_timeout_timer(self):
        ...
        if self.pad_lock.locked() and self.locked_time + self.max_acquire_time < now: 
            self.pad_lock.release()
            self.get_logger().info(f"Lock timed out. Releasing anyway. Current holder: {self.locked_name}")
