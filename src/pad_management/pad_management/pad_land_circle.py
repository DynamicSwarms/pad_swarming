...
class PadLandCircle(Node):
        ...
        self.declare_parameter(
            ...
            descriptor=ParameterDescriptor(
                floating_point_range=[
                    FloatingPointRange(from_value=0.25, to_value=3.0),
                ]
            ),
        )
    ...
    def match_formation(self, name: str):
        ...
        for i, cf_pos in enumerate(positions):
            ...
            dst = [
                np.linalg.norm(cf_pos - pos) for pos in formation
            ]  # Distances of Crazyflie to all formation points
            closest = np.argmin(dst)  # closest formation point index
            second = (
                ((closest + 1) % CNT)
                if dst[(closest + 1) % CNT] <= dst[(closest - 1) % CNT]
                else (closest - 1) % CNT
            )  # closest neighbour index of closest
            # We could have used the two closest, the hope is to have better stability on intersection points

            angles.append(
                self.circular_interpolate(
                    STEPS[closest], STEPS[second], dst[closest], dst[second]
                )
            )  # The shape angle we have
            unit_pos.append(
                np.array([np.sin(angles[i]), np.cos(angles[i])])
            )  # only needed for angle difference calculation
        ...
        a_dfrcs = (
            []
        )  # Angular difference to other crazyflies -> used for seperation calculation
        ...
        dif = [
            abs(self.abs_angle_diff(angle, step)) for step in STEPS
        ]  # Anglular difference to all available points on shape
        i, j = np.argsort(dif)[
            :2
        ]  # index of smallest and second smallest angular difference
    ...
    def seperation_factor(self) -> float:
        return (
            self.get_parameter("seperation_factor").get_parameter_value().double_value
        )
    ...
    def get_angle(self, v, w):
        ...
        rot_matrix = np.array(
            [
                [np.cos(_rad_ang), -np.sin(_rad_ang)],
                [np.sin(_rad_ang), np.cos(_rad_ang)],
            ]
        )
